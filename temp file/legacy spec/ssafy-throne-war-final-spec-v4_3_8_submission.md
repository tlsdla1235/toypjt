...
> **v4.3.7 → v4.3.8 변경 요약 (제출용 범위 선명화)**
>
> - **제출용 기준의 구현 범위를 더 명확히 분리**: 본문에서는 "이번 2주 구현에 실제로 포함되는 것"을 더 앞세우고, 후순위 방어 장치는 부록 C와 `Known Limitations`로 보내 최종 제출본의 메시지를 단순화.
> - **WebSocket의 위치를 "선택 실험"으로 명확화**: Phase 2의 비교 실험 항목이지만, 일정이 밀리면 즉시 제외 가능한 선택 범위임을 Core/요구사항/기술스택에 일관되게 반영.
> - **finalize 엄밀 게이트 표기 정리**: 제출용 구현 기준은 `LLEN==0 + 짧은 drain wait`를 기본으로 하고, `inFlight + DLQ baseline` 3중 게이트는 설계/후속 과제로 남김.
> - **RUNNING 1개 제약 설명 압축**: DB generated column 방식과 애플리케이션 fallback을 "둘 다 검토했지만 실제 구현은 하나만 채택"하는 것으로 정리해 과도한 분기 서술을 줄임.
>
...
| WebSocket 대안 구현 (선택) | 2 | SSE와 동일 기능의 비교 실험. 일정이 밀리면 제외 가능 |
...
- **이번 제출본에서 반드시 보이는 것**: Phase 1 MVP 완성, Phase 2 baseline + 개선 3종, Phase 3 Redis Lua 핫패스 이전과 k6 before/after 비교
- **있으면 좋지만 빠져도 되는 것**: WebSocket 비교, 추가 락 전략 실험, finalize 엄밀 3중 게이트
- **문서로만 남겨도 되는 것**: Consumer 크래시 내성, Redis 전체 유실 자동 복구, 프로덕션 수준 ack/replay 체계

> 제출용 기준의 핵심은 "모든 방어 장치를 다 넣었다"가 아니라, **"어디까지 구현하고 어디서 멈췄는지를 스스로 설명할 수 있다"**이다.
...
### FR-06. WebSocket (Phase 2 선택 비교 구현)

Phase 1에서는 SSE만 제공한다. WebSocket은 **Phase 2의 선택 비교 실험**으로 두며, 일정이 밀리면 구현하지 않아도 전체 서사는 유지된다.

- **FR-06-1**: 구현하는 경우 `/ws/throne` 엔드포인트로 SSE와 동일한 정보 전송.
- **FR-06-2**: 구현하는 경우 STOMP 또는 순수 WebSocket 중 하나를 선택.
- **FR-06-3**: 구현하는 경우 handshake 인터셉터에서 JWT 검증. 운영 기준은 SSE와 동일하게 **HttpOnly 쿠키 기반**을 사용한다.
- **FR-06-4**: 미구현 시 운영 기본값은 SSE로 확정하고, 비교 실험은 후속 과제로 남긴다.
...
|------|------|------|
| Language | Java 17 | |
| Framework | Spring Boot 3.x | MVC + SseEmitter (WebSocket은 선택 실험) |
...
| 실시간 | SseEmitter 기본 / WebSocket 선택 | 비교 실험용 |
...
> **RUNNING 1개 제약의 구현 원칙 (M12 — 제출용 정리)**: `running_marker + UNIQUE`는 **기본안**으로 두되, Day 3 로컬 검증에서 예상대로 동작하지 않으면 **애플리케이션 레이어 fallback**(`SELECT ... FOR UPDATE` 기반 `/start` 검사)으로 대체한다. 중요한 점은 두 방식을 동시에 끌고 가지 않는 것이다. **실제 구현에는 하나만 채택**하고, 나머지는 README에 대안으로만 남긴다.
...
각 개선은 **독립 커밋 + 독립 측정 + 독립 문서화**. 다만 WebSocket은 예외적으로 선택 범위다. Phase 3 진입 전에 중요한 것은 "MySQL + JVM 레벨에서 할 수 있는 핵심 개선은 해봤다"는 점이지, 비교 실험을 전부 완료하는 것 자체가 목적은 아니다.
...
- **제출용 범위 정리**: in-flight 추적과 finalize 엄밀 게이트는 실제 구현 대신 설계/문서로 남긴다. 이 문서의 핵심은 Redis 핫패스 이전과 수치 비교다.
...
public EventSummary finalize(long eventId) {
    ...
    // 0. Phase 3 전용: Consumer backlog drain 대기 (제출용 단순 버전)
    //    Phase 1에서는 Consumer가 없으므로 no-op.
    //    Phase 3 제출본에서는 LLEN queue:throne_events == 0 확인 + 짧은 drain wait만 사용한다.
...
> **Consumer backlog gate (제출용 기준)**: Phase 3 `ConsumerBacklogGate.awaitOrThrow`는 500ms 간격으로 `LLEN queue:throne_events == 0`을 확인하고, 짧은 drain wait 동안 상태가 유지되는지만 본다. 30초 타임아웃 도달 시 `ConsumerBacklogException` (`409 CONSUMER_BACKLOG`)을 반환한다.
>
> Phase 1에서는 no-op 구현체로 주입되어 바로 반환한다. `inFlight + DLQ baseline`을 포함한 엄밀한 3중 게이트는 설계와 근거를 부록 C 및 `Known Limitations`에 남기고, 제출용 구현에서는 제외한다.
...
- ⭐ 일정이 밀리면 **이 날 전체를 생략**하고 SSE를 운영 기본값으로 확정한다. WS 비교는 Day 14 이후 선택 과제로 이관한다.
