...
## Scope For Today
- [ ] `EventParticipant` 엔티티 + `EventParticipantId` 복합키 작성
- [ ] `EventParticipantRepository` 작성 (`applyDeltaBatch`, `findTopByEventId`, `findMostClaims` 준비)
- [ ] `LeaderboardSnapshot` 엔티티 + `LeaderboardSnapshotId` 복합키 작성
- [ ] `LeaderboardSnapshotRepository` 작성 (`saveAllIgnore`)
- [ ] `AggregationCache` 컴포넌트 작성 (`AggregationDelta` record + `pending` ConcurrentHashMap)
- [ ] `AggregationCache.flush()` `@Scheduled(fixedDelay = 10_000)` 작성
- [ ] `AggregationCache.finalFlush()` `@PreDestroy` 작성
- [ ] `AggregationCache.forceFlush()` 동기 진입점 작성
- [ ] `CooldownService.purgeExpired()` `@Scheduled(fixedDelay = 60_000)` 작성
- [ ] `CooldownRepository.deleteExpired(threshold)` 메서드 추가
- [ ] `LeaderboardService.snapshotLeaderboard()` `@Scheduled(cron = "0 */5 * * * *")` 작성
- [ ] `ThroneService.doClaim()` afterCommit에 `addReignEnd` / `addClaimSuccess` 호출 추가
- [ ] Day 6 `finalize` 대비: 마지막 왕의 열린 reign 종료분도 집계 캐시에 반영되도록 후속 TODO 명시

## What Should Be Implemented

### 1. EventParticipant 도메인
- [ ] `EventParticipant` 엔티티 (`event_id`, `user_id` 복합키)
- [ ] 필드: `totalHoldMs`, `claimSuccessCnt`, `longestReignMs`, `joinedAt`, `updatedAt`
...
- [ ] `EventParticipantRepository.findTopByEventIdOrderByTotalHoldDesc(eventId, limit)`
  - `WHERE user_id <> 1` 조건
  - Pageable 또는 setMaxResults로 Top N
  - `ParticipantRow(userId, totalHoldMs, claimSuccessCnt, longestReignMs)` projection 반환
- [ ] `EventParticipantRepository.findMostClaims(eventId)`
  - Day 6 `USURPER` 시상 계산용
  - `WHERE user_id <> 1 ORDER BY claim_success_cnt DESC, user_id ASC`

### 2. LeaderboardSnapshot 도메인
- [ ] `LeaderboardSnapshot` 엔티티 (`event_id`, `rank_no`, `captured_at` 복합키)
- [ ] 필드: `userId`, `totalHoldMs`
- [ ] `LeaderboardSnapshotRepository.saveAllIgnore(rows)`
  - native bulk `INSERT IGNORE INTO leaderboard_snapshot ...`
  - 동일 5분 주기 중복 호출 시 PK 충돌 조용히 skip

### 3. AggregationCache
- [ ] `AggregationDelta(long totalHoldMs, int claimSuccessCnt, long maxReignMs)` record + `merge()` + `ofHold(ms)` / `ofClaim()` 팩토리
- [ ] `Key(long eventId, long userId)` record
- [ ] `pending: ConcurrentHashMap<Key, AggregationDelta>`
- [ ] `SYSTEM_USER_ID = 1L` 상수
- [ ] `addReignEnd(eventId, prevUserId, durationMs)` — `prevUserId == 1L`이면 early return, 아니면 `pending.merge(..., AggregationDelta::merge)`
- [ ] `addClaimSuccess(eventId, userId)` — 동일 패턴, SYSTEM 방어 필터 포함
- [ ] `flush()` `@Scheduled(fixedDelay = 10_000)` — **반드시 `pending.remove(key)` 기반 원자 추출**, SYSTEM 이중 방어 필터, 빈 맵일 때 DB 호출 없이 return
- [ ] `flush()`는 drained map 전체를 `applyDeltaBatch(...)`에 넘긴다 — key별 개별 DB 호출 루프를 기본안으로 삼지 않는다
- [ ] `finalFlush()` `@PreDestroy` — `flush()` 위임
- [ ] `forceFlush()` — `LeaderboardService`에서 동기 호출용, `flush()` 위임

### 4. CooldownService
- [ ] `CooldownService.purgeExpired()` `@Scheduled(fixedDelay = 60_000)` + `@Transactional`
- [ ] `cooldownRepository.deleteExpired(now.minusSeconds(10))` 호출
- [ ] `CooldownRepository`에 `@Modifying @Query("delete from Cooldown c where c.expiresAt < :threshold") int deleteExpired(...)` 추가

### 5. LeaderboardService
- [ ] 의존성: `EventService`(또는 `EventRepository`), `EventParticipantRepository`, `LeaderboardSnapshotRepository`, `AggregationCache`
- [ ] `snapshotLeaderboard()` `@Scheduled(cron = "0 */5 * * * *")`
...
### 6. ThroneService 연동
- [ ] `ThroneService` 생성자 주입 필드에 `AggregationCache` 추가
- [ ] `doClaim()` 마지막 `runAfterCommit(...)` 블록에 두 줄 추가:
  ```
  aggregationCache.addReignEnd(eventId, currentState.currentKingId(), heldMs);
  aggregationCache.addClaimSuccess(eventId, userId);
  ```
- [ ] 첫 찬탈(이전 왕 = SYSTEM)도 `addReignEnd`는 호출되지만 내부에서 early return으로 무시
- [ ] Day 6 `finalize` 구현 시, 이벤트 종료 시점까지 왕좌를 들고 있던 마지막 유저의 열린 reign duration도 `addReignEnd`로 반영해야 함
...
- [ ] `EventParticipant` 엔티티 + `EventParticipantId`
- [ ] `EventParticipantRepository` (`applyDeltaBatch`, `findTopByEventIdOrderByTotalHoldDesc`, `findMostClaims`)
- [ ] `LeaderboardSnapshot` 엔티티 + `LeaderboardSnapshotId`
- [ ] `LeaderboardSnapshotRepository` (`saveAllIgnore`)
- [ ] `CooldownRepository.deleteExpired(LocalDateTime threshold)` 추가

### Service support
- [ ] `AggregationCache` 컴포넌트 (write-back 핵심)
- [ ] `CooldownService` (1분 주기 만료 정리)
- [ ] `LeaderboardService` (5분 주기 스냅샷)
- [ ] `ThroneService` → `AggregationCache` 단방향 호출
- [ ] `LeaderboardService` → `AggregationCache.forceFlush()` 단방향 호출

### Migration
- [ ] 신규 Flyway 마이그레이션 **불필요** (V1__init_schema.sql에 `event_participants`, `leaderboard_snapshot` 모두 존재)

## Behavior Rules To Keep
- [ ] SYSTEM(`user_id = 1`) row는 절대로 `event_participants` / `leaderboard_snapshot`에 들어가지 않는다 (4중 방어: `addReignEnd`, `addClaimSuccess`, `applyDelta`, `findTop...`의 `user_id <> 1`)
- [ ] `flush()`는 반드시 `pending.remove(key)` 원자 추출 사용 — 단순 `clear()` 또는 iterate-after-snapshot 금지 (race로 신규 누적분 유실)
- [ ] `flush()`는 빈 맵일 때 DB 호출 없이 return
- [ ] write-back의 기본 단위는 "flush 1회당 drained map 전체"다 — 참가자별 개별 upsert 반복을 기본 구조로 두지 않는다
- [ ] `purgeExpired()`는 `now - 10s` 기준 (시간 동기 오차 grace)
- [ ] `snapshotLeaderboard()`는 RUNNING 이벤트 없으면 no-op
- [ ] `snapshotLeaderboard()`의 `captured_at`은 5분 경계로 truncate (M11 — GC pause로 실행 시각이 흔들려도 같은 PK)
- [ ] `INSERT IGNORE`로 중복 호출 시 조용히 skip (I5)
- [ ] 찬탈 트랜잭션 롤백 시 `addReignEnd` / `addClaimSuccess` 호출 안 됨 (afterCommit 안에 있음)
- [ ] 찬탈 핫패스(`doClaim`)는 DB 집계 UPDATE를 직접 하지 않는다
- [ ] Day 6 finalize는 열린 마지막 reign을 닫은 뒤 그 duration을 집계 캐시에 반영해야 한다
...
- [ ] `addReignEnd(SYSTEM)`은 pending에 들어가지 않는다
- [ ] 같은 키로 `addReignEnd`를 두 번 호출하면 `totalHoldMs`가 합산된다
- [ ] `maxReignMs`는 `GREATEST` 의미로 누적된다 (작은 값 뒤 큰 값 → 큰 값 유지)
- [ ] `flush()` 호출 후 pending이 비워지고 `applyDelta`가 키별로 호출된다
- [ ] `flush()` 호출 중 `addReignEnd`가 들어와도 신규 분이 유실되지 않는다
- [ ] `finalFlush()`가 `flush()`를 위임한다
- [ ] pending이 비어있으면 `applyDelta`가 한 번도 호출되지 않는다
- [ ] drained map이 한 번의 `applyDeltaBatch(...)` 호출로 전달된다
...
- [ ] RUNNING 이벤트 없으면 `forceFlush` / `applyDelta` / `saveAllIgnore` 모두 호출되지 않는다
- [ ] RUNNING 이벤트 있으면 `forceFlush()` → `findTop...` → `saveAllIgnore` 순서로 호출된다
- [ ] `captured_at`이 5분 경계로 truncate된다 (`12:03:42` → `12:00:00`)
- [ ] `rank_no`가 1부터 순차 채워진다

### ThroneService 통합 테스트 (기존 테스트 확장)
- [ ] 찬탈 성공 + commit 시 `aggregationCache.addReignEnd(prevUserId, heldMs)`가 호출된다
- [ ] 찬탈 성공 + commit 시 `aggregationCache.addClaimSuccess(newUserId)`가 호출된다
- [ ] 첫 찬탈(이전 왕 = SYSTEM)이라도 `addReignEnd` 호출 자체는 정상 (내부에서 early return)
- [ ] 트랜잭션 롤백 시 `addReignEnd` / `addClaimSuccess`가 호출되지 않는다 (afterCommit 보장)
  - 이 항목은 mock 기반 단위 테스트만으로는 부족할 수 있으므로 실제 transaction synchronization이 있는 통합 테스트로 검증
- [ ] `COOLDOWN`, `ALREADY_OWNER`, `NOT_RUNNING` 실패 시 집계 호출 없음
...
- [ ] 코드가 빌드된다
- [ ] `AggregationCache`가 10초 주기로 메모리 → DB write-back을 수행한다
- [ ] `pending.remove` 기반 race-safe flush가 동작한다
- [ ] `@PreDestroy`로 셧다운 시 누적분이 보존된다
- [ ] SYSTEM 유저가 `event_participants` / `leaderboard_snapshot`에 절대 들어가지 않는다 (4중 방어)
- [ ] 만료 쿨타임이 1분 안에 삭제된다
- [ ] 5분 주기 리더보드 스냅샷이 기록된다
- [ ] 동일 5분 구간 중복 호출이 PK 충돌로 조용히 skip된다
- [ ] 찬탈 핫패스(`doClaim`)가 DB 집계 UPDATE를 직접 하지 않는다
- [ ] 본인이 흐름을 설명할 수 있다
